function summaryTable = getSummaryInfo(trial_list, fs, outputLabel, bodyPart, processlabel, trialIndex)
	...
	for trial_idx = 1:length(trial_list)
		...
		distRelMaxSpeed=trial.distDataLabel.distDataLabel(speedMax_idx)
        ...
        wrist_endpoint_trial=trial.distDataLabel.poseIDAngleRefPosition(endPos);
        ...
        start_AP_trial = trial.distDataLabel.([poseIDAngleRefPosition(1).xColLabel]);
        start_AP_trial =start_AP_trial(1);
        start_ML_trial = trial.distDataLabel.([poseIDAngleRefPosition(1).yColLabel]);
        start_ML_trial= start_ML_trial(1);
        start_DV_trial = trial.distDataLabel.([poseIDAngleRefPosition(1).zColLabel]);
        start_DV_trial=start_DV_trial(1);
        start_r_trial=trial.distDataLabel.poseIDAngleRefPosition;
        start_r_trial=start_r(1);
        ...
        tortuosity_trial=getTortuosity(trial,distDataLabel,[poseIDAngleRefPosition,rColLabel]);
